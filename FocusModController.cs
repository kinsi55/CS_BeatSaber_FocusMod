...
//		MonoBehaviour scoreElement = null;
//		IEnumerable<GameObject> countersElements = null;
...
//				var idk = Resources.FindObjectsOfTypeAll<Canvas>().Where(xd => xd.transform.childCount != 0 && xd.name.StartsWith("Counters+ |")).LastOrDefault();

//				countersElements = idk.transform.Cast<Transform>().Select(x => x.gameObject);

//				scoreElement = Resources.FindObjectsOfTypeAll<ImmediateRankUIPanel>().LastOrDefault();
//				if(scoreElement == null || scoreElement.transform.childCount == 0) {
//					scoreElement = Resources.FindObjectsOfTypeAll<ScoreUIController>().LastOrDefault();
...
//			var counterClasses = (new string[] {
//				"CountersPlus.Counters.Custom.",
//				"CountersPlus.Counters.Counter`1"
//			});

//			//otherElementLookups = AppDomain.CurrentDomain.GetAssemblies()
//			//	.SelectMany(assembly => assembly.GetTypes())
//			//	.Where(type => type.IsClass && type.BaseType?.FullName != null && counterClasses.Any(name => type.BaseType.FullName.StartsWith(name)));
//		}
//		/// <summary>
...
//			if(scoreElement == null || audioTimeSyncController == null)
//				return;
...
//						if(countersElements != null) foreach(var elem in countersElements)
//							elem.SetActive(true);
